...
def exp(log_r: Array) -> Array:
    ...
    R = I + sin_angle[..., None] * K + (1.0 - cos_angle)[..., None] * jnp.matmul(K, K)
...
def log(R: Array) -> Array:
    ...
    #   ▸ small-angle branch will not use sin / angle at all
    sin_angle = jnp.where(small_angle, 1.0, jnp.sin(angle))
    ...
    # --------  New stable formulation  --------
    w_small = 0.5 * skew_part  # θ≈0  →  log R ≈ ½(R−Rᵀ)∨
    w_general = angle[..., None] * axis_general  # regular case
    w_pi = angle[..., None] * axis_pi  # θ≈π case

    return jnp.where(small_angle[..., None], w_small, jnp.where(near_pi[..., None], w_pi, w_general))
...
def to_quaternion(matrix: Array) -> Array:
    ...
    q0 = jnp.stack([trace + 1.0, m21 - m12, m02 - m20, m10 - m01], axis=-1) * 0.5  # Scale by 0.5 for correct gradients

    q1 = jnp.stack([m21 - m12, m00 - m11 - m22 + 1.0, m01 + m10, m02 + m20], axis=-1) * 0.5  # Scale by 0.5

    q2 = jnp.stack([m02 - m20, m01 + m10, m11 - m00 - m22 + 1.0, m12 + m21], axis=-1) * 0.5  # Scale by 0.5

    q3 = jnp.stack([m10 - m01, m02 + m20, m12 + m21, m22 - m00 - m11 + 1.0], axis=-1) * 0.5  # Scale by 0.5
